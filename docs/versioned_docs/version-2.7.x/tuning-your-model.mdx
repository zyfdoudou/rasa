...
abstract: Rasa Open Source will provide you with a suggested NLU config on initialization of the project, but as your project grows, it's likely that you will need to adjust your config to suit your training data.
---
...
```yaml-rasa (versioned_docs/version-2.7.x/sources/data/configs_for_docs/default_spacy_config.yml)
```
...
```yaml-rasa (versioned_docs/version-2.7.x/sources/data/configs_for_docs/default_config.yml)
```
...
```yaml-rasa (versioned_docs/version-2.7.x/sources/data/configs_for_docs/config_featurizers.yml)
```
...
To do this, you need to access the `diagnostic_data` field of the REPLACED LINK
and REPLACED LINK objects, which contain
